...
class WitnessCalculator {
  ...
  async _doCalculateWitness(input, sanityCheck) {
    ...
    keys.forEach( (k) => {
      ...
      if (fArr.length > signalSize) {
        throw new Error('Too many values for input signal');
      }
...
window.addEventListener('message', async (message) => {
  ...
  if (data.event === '@EXECUTE_WASM') {
    const binary = base64ToArrayBuffer(data.binary);

    const build = await builder(binary);

    let witnessCalculationResult;
    try{
      const witnessCalculation = await build.calculateWTNSBin(data.inputs);
      witnessCalculationResult = arrayBufferToBase64(witnessCalculation);
    } catch(e) {
      alert(e);
    }

    window.ReactNativeWebView.postMessage(JSON.stringify({witnessCalculationResult, event: '@EXECUTION_RESULT'}));
