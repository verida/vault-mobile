import {InboxInstance, OutboxInstance} from "./inbox/Inbox";

// @todo: Move Verida interfaces into a separate package and import

export interface Client {
    openContext(contextName: string, forceCreate: boolean): Promise<Context | undefined>
    openPublicProfile(did: string, contextName: string)
}

export interface Context {
    getMessaging(): Promise<Messaging>
    openProfile(profileName: string, did?: string): Promise<Profile | undefined>
    openDatabase(databaseName: string, options: DatabaseOpenConfig): Promise<Database>
