...
export class InboxManager {
     ...
     * @param payload
     */
    public async handleAction (inboxEntry: InboxEntry, action: keyof DataAction, payload: any) {
        await this.init()
        ...
        const MiddlewareInstance = new Middleware(this.vaultCommon, this.messaging, inboxEntry, payload)
    ...
    public async fetchMessageMetadata(inboxEntry: InboxEntry) {
        ...
        let senderProfile
        try {
            const sender = await this.vaultCommon.client.openProfile("basicProfile", senderDid, senderApp)
        ...
        const MiddlewareInstance = new Middleware(this.vaultCommon, this.messaging, inboxEntry)
