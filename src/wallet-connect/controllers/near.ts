...
export class NearWalletController {
  ...
  static async init(networkId: string) {
    ...
    const accounts = (
      await Promise.all(
        allAccounts.map(async (accountId) => {
          const keypair = await keyStore.getKey(networkId, accountId)
          const publicKey = keypair.getPublicKey().toString()
          return publicKey === nearWallet.publicKey ? accountId : null
        })
      )
    ).filter(Boolean)
    ...
    return new NearWalletController(networkId, keyStore)
  }
    ...
    const accounts = (
      await Promise.all(
        accountIds.map(async (accountId) => {
          ...
          const publicKey = keyPair.getPublicKey().toString()
          return publicKey === nearWallet.publicKey
            ? {
                accountId,
  ...
  private async isAccountsValid(
    topic: string,
    accounts: Array<{ accountId: string }>
  ) {
    const signClient = await getWC2SignClient()
    const session = signClient.session.get(topic)
    const validAccountIds = session.namespaces.near.accounts.map(
      (accountId) => {
  ...
  async signIn({
    ...
    permission,
    accounts,
  }: SignInParams): Promise<Array<Account>> {
  ...
  async signOut({
    ...
    topic,
    accounts,
  }: SignOutParams): Promise<Array<Account>> {
    if (!this.isAccountsValid(topic, accounts)) {
      throw new Error('Invalid accounts')
    }
  ...
  async signTransactions({
    ...
    topic,
    transactions,
  }: SignTransactionsParams): Promise<
    Array<nearTransactions.SignedTransaction>
  > {
  ...
  async signAndSendTransactions({
    ...
    topic,
    transactions,
  }: SignAndSendTransactionsParams): Promise<
